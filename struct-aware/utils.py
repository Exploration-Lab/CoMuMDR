...
def tsinghua_F1(eval_matrix):
    ...
    for hypothesis, reference, edu_num in zip(eval_matrix['hypothesis'], eval_matrix['reference'],
        ...
        # for r in reference:
        #     cnt[r[1]] += 1
        cnt = [0] * (edu_num + 1)  # Ensure cnt is large enough to handle all indices
        for r in reference:
            if r[1] < len(cnt):  # Safeguard against out-of-range indices
                cnt[r[1]] += 1
            else:
                print(f"Warning: Out-of-range index in reference: {r[1]} (edu_num={edu_num})")
