...
def batch_bert_variable(onebatch, config, tokenizer):
    ...
    batch_input_ids = np.ones([batch_size, max_edu_num, max_tok_len], dtype=np.int64) * tokenizer.pad_token_id()
    batch_token_type_ids = np.zeros([batch_size, max_edu_num, max_tok_len], dtype=np.int64)
    batch_attention_mask = np.zeros([batch_size, max_edu_num, max_tok_len], dtype=np.int64)
...
def batch_sp_variable(onebatch, vocab):
    ...
    batch_sp = np.zeros([batch_size, max_edu_len], dtype=np.int64)
