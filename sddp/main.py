...
from CoMuMDR.structured_dialogue_discourse_parsing.dataset import SelectionDataset
...
def eval_running_model(dataloader, test_mode):
    ...
    for ds, g, r in zip(tree_results, gt, relation_types):
        ...
        if args.link_only:
            ...
            # get all the predicted relations and gold relations to make a confusion matrix
            d_links = set([dd[:2] for dd in d])
            g_links = set([gg[:2] for gg in g])
            for link in d_links:
                if link in g_links:
                    predicted_types.append([dd[2] for dd in d if dd[:2] == link][0])
                    gold_types.append([gg[2] for gg in g if gg[:2] == link][0])
    ...
    relation_type_confusion_matrix = confusion_matrix(gold_types, predicted_types)
    print(relation_type_confusion_matrix)
    return {'f1':f1, 'relation_type_confusion_matrix':relation_type_confusion_matrix}
