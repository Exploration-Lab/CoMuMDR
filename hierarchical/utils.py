...
#     print("Loading word vectors using XLM-RoBERTa...")
    
#     # Load XLM-RoBERTa tokenizer and model
#     tokenizer = AutoTokenizer.from_pretrained(FLAGS.embedding_model)
#     model = AutoModel.from_pretrained(FLAGS.embedding_model)
...
#         # Use the first token's embedding (pooled output is not available for XLM-R)
#         token_embedding = outputs.last_hidden_state.mean(dim=1).squeeze(0).numpy()

#         embed.append(token_embedding)
#         cnt_pretrained += 1
...
#         std = np.zeros((len(dialog["edus"]), len(dialog["edus"])))
#         for relation in dialog["relations"]:
#             std[relation["x"]][relation["y"]] = relation["type"] + 1  # +1 to handle zero-indexing
...
#         for relation in dialog["relations"]:
#             true_labels.append(relation["type"])
#         for relation in pred[i]:
#             predicted_labels.append(relation[2])
...
#     # Ensure lengths of true_labels and predicted_labels are consistent
